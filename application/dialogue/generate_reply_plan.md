...
> - **Drafting beat or dialogue content.** Sentences like *"Hana beat 2: 'Says please before knowing what she is asking for...'"* or *"Saki's first line could be: 'You came at contact...'"* belong inside the `Write` call's JSON, not in thinking. If you find yourself composing beat strings, stop and write them.
> - **Manual word counting.** Sentences like *"= 16 words ✓"*, *"= 33 words, way over"*, *"19 words ✓"* are forbidden. `check_beat_sizing.py` runs mechanically after you. Spot-check obvious overflows by eye only, do not iterate counts.
> - **Process narration.** *"Now I'm constructing..."*, *"Now I'm verifying..."*, *"Now I'm moving into..."* — all forbidden. Decide and emit, do not announce yourself.
> - **Re-checking the same constraint multiple times.** If you find yourself returning to the same rule three or four times, you are stuck in a loop. Escalate by writing the plan and trusting the validator. Looping in thinking is the most expensive failure mode.
> - **Restating last_turn or recent_chat content into thinking.** The files are in your context. Do not paraphrase or summarize them inside thinking.
>
> **Permitted inside thinking blocks (terminal creative decisions only):**
> - Who speaks this round and in what order (apply the rules in section 3)
> - What beat structure each speaker's turn carries (weight + beat skeleton)
> - Scene anchor extraction (which fields go where)
> - TBC framing if applicable
> - How to scope user_prompt direction

> **Read all Required-reads files in a single parallel batch.** Do not split reads across multiple thinking blocks.
...
**Critical: `user_prompt` framing is PLACEMENT, not EXCLUSION.** A prompt like `"Rosa's turn, <weight>: <direction>"`, `"Alice does X"`, or `"describe Bob's reaction"` names one character as the round's center of gravity — that character becomes the `round_protagonist`, Tier 1 auto-includes, and goes **first** in `turn_order` (see 3b). It does **NOT** exclude other participants from the round. Every other participant still gets evaluated normally — they are included when their own qualifications are met (physically engaged, directly addressed, reacted-to, primary reactor, silence fatigue, etc.). A single-character round is correct only when the user_prompt explicitly excludes others (e.g. `"Rosa-only monologue"`, `"other characters stay silent"`) OR when genuinely no other participant has a reactive beat available. If the protagonist's turn ends on a physical event landing on another character's body (touch, contact, strike, gesture toward), that character is "physically engaged" and must speak — the prompt naming the protagonist does not override that.

**Player character exclusion (when `player_id` is set in `characters.json`):** If `player_id` is set and the player character is the most recent speaker in `recent_chat.json` (their line was just typed by the user), exclude them from the speaking set entirely — do not plan a turn for them. Their line is already in the chat. Plan reactions from the other participants only. If the player character is NOT the most recent speaker (rare — e.g. a "Skip" round), include or exclude them at your discretion per normal selection.

**For 2-participant scenes (the common case):** Both participants speak unless (a) `user_prompt` explicitly excludes one, or (b) one genuinely has no reactive beat available. ROUND METADATA already gates this — skip detailed tier evaluation.

**For 3+-participant scenes:** Use the full Tier 1/2/3 evaluation in **Addendum B** at the end of this file.
...
**TBC resumer override:** if `tbc.json` exists, the TBC speaker is **always first** in `turn_order`. See **Addendum A** for the full TBC contract.

**User prompt override:** if `user_prompt` is primarily about one character — whether it directs their action ("Alice does X"), describes something happening to or from their body ("describe Bob's X"), or frames an event from their perspective — that character goes **first** in `turn_order` regardless of default placement. This is placement-only — see the 3a "PLACEMENT, not EXCLUSION" rule. (If both a TBC resumer AND a user prompt override apply to different characters, the TBC resumer wins — the frozen action must resolve before new direction can be applied.)

### 3c. TBC framing

If `tbc.json` exists at plan time, OR you are considering ending this round on a TBC, see **Addendum A — TBC handling contract**. Otherwise no TBC work is needed this round.
...
- **`ends_tbc`** (optional, at most one per round, last turn only) — set to `true` only if this turn ends on a deliberate to-be-continued freeze. See **Addendum A**.
...
> **`scene_anchor` is LOAD-BEARING.** This is the non-negotiable carryover from the prior round's end-state. **`scene_state.json` is the canonical persistent record** — it carries forward time, location, proximity, positions, wardrobe, in_progress_action across rounds, so details that weren't explicitly mentioned in last_turn's prose don't get lost. Validator check `2h_scene_anchor_contradiction` cross-checks the anchor against scene_state + last_turn; disagreement triggers instant replan (critical).
>
> **Your job — read-only on subsequent rounds, seed once on round 1:**
> - **If `scene_state.json` exists** (round 2+): read it and mirror `current_state` into your plan's `scene_anchor` field exactly. Do NOT update or rewrite `scene_state.json` — Phase 3b actualizer (post-merge) handles all updates based on the actual prose written this round. Updating it from the planner side would race the actualizer and silently drop deltas, since you're reasoning about plans not actual prose.
> - **If `scene_state.json` does NOT exist** (round 1, the first ever round): seed it. Build `current_state` from `last_turn.json` (the greeting) + `context_cache.scenario` for fields the greeting didn't explicitly establish. The greeting always wins where it conflicts with scenario defaults. Write the initial `scene_state.json` with `rounds_processed: 1` and `version: 1`. Then mirror `current_state` into `scene_anchor` and proceed. Schema: `{dialogue_id, version, rounds_processed, current_state: {time, location, proximity, positions, wardrobe, in_progress_action}, last_updated_round}`.
>
> **Why this split:** state deltas need interpretation of what actually happened in prose ("did the wardrobe shift? did time advance?"), which is reasoning work — not deterministic. The actualizer runs post-merge with full prose in hand and is the right place. The planner only ever needs to *read* the running truth, never patch it (except the one-time seed).

For concrete `reaction` and `inflection` turn examples, see the **Reference examples** section at the end of this file.
...
Spot-check only — `validate_plan.md` and `check_beat_sizing.py` run mechanical validation after you. Do NOT iterate word counts in thinking.

**Structural integrity:**
- [ ] `turn_order` entries are all valid participant ids; `turn_order[i] == turns[i].speaker` for every i
- [ ] At least one turn present; `round_protagonist` is set and is a member of `turn_order`
- [ ] Every turn has `weight` (one of `reaction` / `action` / `inflection` / `climax`), `beats` array, `tone` string
- [ ] `len(beats)` within weight cap (reaction 1-2, action 2-3, inflection 3-4, climax 4-6)
- [ ] No beat obviously over 25 words (validator counts mechanically — only flag obvious overflows)
- [ ] No participant speaks more than once unless the scene clearly demands it
- [ ] `character_briefs` is NOT in your output (sidecar, built by Phase 0)

**Scene anchor (load-bearing):**
- [ ] `scene_anchor` populated with all six fields (`time`, `location`, `proximity`, `positions`, `wardrobe`, `in_progress_action`). `positions` and `wardrobe` keyed by every participant's `char_id`.
- [ ] No planned beat contradicts `scene_anchor` — no time reversal, no teleporting, no wardrobe drift, no proximity jump. If a transition is needed, it MUST appear as an explicit beat in turn 0 before any state-dependent beat.

**TBC (only if relevant — see Addendum A):**
- [ ] If `tbc.json` exists: `speaker` is `turn_order[0]`, is `round_protagonist`, has `ends_tbc: false`, AND no other turn has `ends_tbc: true` (resumer rule).
- [ ] If `ends_tbc: true` on any turn: it is the **last** turn, has populated `tbc_state`, and top-level `pending_tbc` matches.

**Player exclusion (only if `player_id` is set):**
- [ ] If `player_id` is the most recent speaker, they do NOT appear in `turn_order` or `turns[]`.

**Output:**
- [ ] Written to `infrastructure/dialogues/{dialogue_id}/reply_plan.json`

---

## Addendum A — TBC handling contract

Read this only when `tbc.json` exists at plan time, OR you are considering ending the round on a TBC.
...
- Plan proceeds as usual. You MAY optionally end this round on a TBC by marking the final plan turn with `ends_tbc: true` and populating `tbc_state` — see "When to mark a turn `ends_tbc: true`" below.

**If `tbc.json` EXISTS at plan time (resumption case):**
- Read it. It has `{speaker, tbc_state, frozen_at}`.
- The TBC speaker is **`turn_order[0]`** — mandatory.
...
**Precedence — TBC resumer beats "last speaker last":** When `tbc.json` exists and the TBC speaker is also the prior round's most recent speaker (the **common** case — TBCs almost always end the prior round, so the resumer is usually also the last speaker), the TBC resumer rule wins outright. The TBC speaker is `turn_order[0]`. The default "most recent speaker goes last" rule from section 3b does NOT apply this round — the TBC has restructured the round. Other speakers go after the resumer in whatever order serves the resumed action. The orchestrator's ROUND METADATA "most recent speaker" hint is informational only when a TBC is active. Do not loop in thinking trying to satisfy both — the TBC override is explicit and authoritative.

### When to mark a turn `ends_tbc: true`
...
## Addendum B — Tier evaluation (3+ participant scenes only)

Read this only when the scene has 3 or more participants. For 2-character scenes, both speak by default — see section 3a.
...
- **TBC resumer:** If `tbc.json` exists, its `speaker` is a Tier 1 auto-include for this round (see Addendum A for the full contract).
...
## Reference examples (informational — do not re-read every spawn)
